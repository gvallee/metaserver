...
func main () {
	...
	if (myFS == nil) { log.Fatal ("ERROR: ClientInit() failed") }

	/* Prepare the buffer to write */
	fmt.Println ("Writing 2000 bytes...")
	buff1 := make ([]byte, 2000)
	if buff1 == nil { log.Fatal ("ERROR: Cannot allocate buffer") }
	...
	if (ws != 2000 || writeerr != err.NoErr) { log.Fatal ("ERROR: Write op failed") }

	fmt.Println ("Writing 500 bytes...")
	buff3 := make ([]byte, 500)
	if buff3 == nil { log.Fatal ("ERROR: Cannot allocate buffer") }
	...
	if (ws != 500 || writeerr != err.NoErr) { log.Fatal ("ERROR: Write op failed") }

	fmt.Println ("Reading 1200 bytes...")
	buff2, readerr := myFS.Read ("default", 1200)
	if (readerr != err.NoErr || uint64 (len (buff2)) != 1200) { log.Fatal ("ERROR: Read operation failed") }

	// We check the content of the buffer
	for i := 0; i < len (buff2); i++ {
		value := int (buff2[i])
		if (value != 3) { log.Fatal ("ERROR: read ", value, " instead of 3 at index ", i) }
	}

	fmt.Println ("Reading 1300 bytes...")
	buff4, readerr := myFS.Read ("default", 1300)
	if (readerr != err.NoErr || uint64 (len (buff4)) != 1300) { log.Fatal ("ERROR: Read operation failed") }

	// We check the content of the buffer
	var i int = 0
	for i = 0; i < 800; i++ {
		value := int (buff4[i])
		if (value != 3) { log.Fatal ("ERROR: read ", value, " instead of 3 at", i) }
	}
	for ; i < 1300; i++ {
		value := int (buff4[i])
		if (value != 7) { log.Fatal ("ERROR: read ", value, " instead of 7 at", i) }
	}
